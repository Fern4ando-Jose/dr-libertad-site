...
export function buildLiteralDirective(
  isLiteral: boolean,
  freedom: string,
  opts?: { lang?: "es" | "br"; anchorBr?: string },
): string {
  if (!isLiteral) return "";
  const lang = opts?.lang ?? "es";
  const anchorBr = opts?.anchorBr?.trim();
  ...
  if (lang === "br" && anchorBr) {
    return `
TEMA-CONVICÇÃO (INVIOLÁVEL) — este tema É uma frase-verdade do autor, e a versão CANÔNICA em português é: «${anchorBr}». A força está nas palavras EXATAS desta frase em português — NÃO traduza o "Tema" em espanhol; use a frase canônica dada:
...
- O PRIMEIRO elemento de "slides" contém a frase canônica COMPLETA, textual: «${anchorBr}».
...
`;
  ...
  const brTranslationRule = lang === "br"
...
- El PRIMER elemento de "slides" contiene la frase COMPLETA del tema, textual.${brTranslationRule}
...
// próprio topic; PT = literalBr), o 1º slide TEM de conter a frase-âncora
...
export function anchorForLang(topic: string, lang: "es" | "br", anchorBr?: string): string | null {
  if (lang === "br") return anchorBr?.trim() || null;
  return topic;
}
