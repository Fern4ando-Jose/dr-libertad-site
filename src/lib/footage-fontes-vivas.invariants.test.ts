...
// Estes testes travam as 5 promessas dessa mudança:
//   1. as 4 fontes são REALMENTE alcançáveis sem a whitelist ter falhado;
...
beforeEach(() => {
  ...
  vi.stubEnv("FOOTAGE_LIVE_SLOTS", ""); // default (2)
  linhasDoBanco = [];
  whoPorUrl.clear();
  URL_POR_ID.clear();
...
describe("1. as 4 fontes funcionam no caminho NORMAL (a whitelist NÃO falha)", () => {
  ...
  it("com a whitelist CHEIA, a busca ao vivo é chamada mesmo assim (era o bug)", async () => {
    ...
    expect(clips.filter(éAoVivo)).toHaveLength(2); // FOOTAGE_LIVE_SLOTS default
  });
  ...
  it("a CAPA (cena 1) continua vindo do acervo CURADO — o ao vivo entra depois", async () => {
    indexarOferta();
    juizAprovaTudo();
  ...
  it("reservar cenas NÃO reembaralha a parte curada (prefixo do sorteio de sempre)", async () => {
    indexarOferta();
    juizAprovaTudo();
...
describe("2. FOOTAGE_LIVE_SLOTS=0 → comportamento IDÊNTICO ao de antes", () => {
  ...
  it("liveSlotsFor: default 2, clampado, e lixo no env → default (fail-open)", () => {
    expect(liveSlotsFor(5, undefined)).toBe(2);
    expect(liveSlotsFor(5, "")).toBe(2);
    expect(liveSlotsFor(5, "0")).toBe(0);
    expect(liveSlotsFor(5, "3")).toBe(3);
    expect(liveSlotsFor(5, "99")).toBe(4); // nunca zera a whitelist (colchão do fail-open)
    expect(liveSlotsFor(5, "-4")).toBe(0);
    expect(liveSlotsFor(5, "abc")).toBe(2);
    expect(liveSlotsFor(1, "2")).toBe(0); // 1 cena → tudo curado
  });
});
