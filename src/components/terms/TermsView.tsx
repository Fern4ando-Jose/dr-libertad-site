...
// Termos de Uso — mesma vestimenta da Política de Privacidade (PrivacyView):
// são páginas irmãs da família "LEGAL" e devem ler igual. A diferença é que
// aqui a copy chega por PROP, não pelo useLang(): esta view também serve a rota
// fixa /terms (EN), que vive fora do LanguageProvider.

import { motion } from "framer-motion";
import StudioContainer from "@/components/ui/Container";
import type { TermsCopy } from "./terms.content";

export default function TermsView({ copy }: { copy: TermsCopy }) {
  return (
    <motion.main
      ...
      <section className="relative overflow-hidden border-b border-warm-gray/10">
        ...
        <StudioContainer>
          <div className="relative pt-32 pb-12 md:pt-36">
            ...
            <div className="mt-4 text-sm text-warm-gray/70">{copy.updated}</div>
      ...
      <section className="py-14 md:py-20">
        <StudioContainer>
          <div className="max-w-3xl">
            <p className="prose-justify text-[1.05rem] leading-[1.85] text-warm-gray/90">
              {copy.intro}
            </p>
