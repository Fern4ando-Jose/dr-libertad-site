...
cima do vídeo. Hoje cada **tema** (os 45 de `THEMES`) tem **som próprio**: o pipeline
...
- **Geração (author-time, paga, 1× por tema):** `scripts/generate-music.mjs` lê os
  45 temas de `THEMES` (em `src/app/api/publish/route.ts` — fonte única, não duplica),
...
FAL_KEY=... node scripts/generate-music.mjs --force      # regenera TODOS os 45 (~US$2,25)
...
(45 → ~US$2,25). Depois é zero — o CI usa os mp3 + manifest commitados. Requer
...
quando os 45 temas estiverem gerados.
