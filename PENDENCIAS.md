...
3. **Mergear** o PR `feat/multi-idioma` na `main` (você revisa primeiro). **SEGURO**:
   os crons dos 3 workflows PT estão **comentados** (dispatch-only) — mergear NÃO
   publica PT sozinho. ES intocado (lang default = es).
4. **Validar a copy PT (sem publicar)** — chaves Anthropic/Tavily são *Sensitive* na
   Vercel, só rodam em **produção**; por isso a validação é **pós-merge**:
   - Eu rodo `/api/publish?preview=1&lang=pt&run=0` em produção e te mostro **PT vs ES**.
   - **Conferir que a copy é BR-NATIVA, não tradução do ES** — gancho, referências e
     jeito de falar brasileiros (vem do `marketBrief` em `accounts.ts`). Se soar
     importado, ajustar o brief (campo de texto, sem mexer em lógica).
   - Depois, opcional: dispatch manual do Reel PT com `publish: no` p/ ver o mp4.
4b. **RELIGAR os crons PT** (commit mínimo: descomentar `schedule:` nos 3 `*-pt.yml`)
